// Image adjustment and dithering for reTerminal E1002 measured-palette tests.
//
...
enum DitherMethod {
  DITHER_NONE = 0,      // Direct nearest color. / 直接最近色
  DITHER_BAYER8,        // Ordered Bayer 8x8. / 有序 Bayer 8x8
...
struct ImageAdjustSettings {
  float exposure;
  float saturation;
  float vibrance;
  bool scurve_enabled;
  float scurve_strength;
  float scurve_shadow;
  float scurve_highlight;
  float scurve_midpoint;
  bool local_contrast_enabled;
  float local_contrast;
  int local_contrast_radius;
};

// Applies optional RGB image preparation in place.
// 原地应用可选的 RGB 图像预处理。
