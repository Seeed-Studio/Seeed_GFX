/*
 * reTerminal E1002 -- measured-palette 6-color SD image comparison.
 *
 * Pipeline:
 *   SD JPG/BMP/PNG -> RGB888 -> optional RGB adjustment
 *               -> measured-palette E6 dither
...
// Error diffusion strength. 1.0 = full Floyd-Steinberg error diffusion.
// Lower values reduce the visible grain, higher values push stronger color mixing.
static const float DITHER_ERROR_STRENGTH = 1.0f;

// Brightness gamma. 1.0 = neutral, >1.0 darkens, <1.0 brightens. Typical 0.8 - 1.6.
static const float DITHER_GAMMA = 1.0f;

// ----- measured-palette pre-processing ---------------------------------------
// These settings mirror the external photo preparation script. Neutral values
// keep the decoded image unchanged before palette mapping.
static const bool RAW_DITHER_ONLY = false;
static const ImageAdjustSettings IMAGE_ADJUST = {
  1.03f,  // exposure
  1.30f,  // saturation
  0.20f,  // vibrance
  true,   // scurve_enabled
  0.90f,  // scurve_strength
  0.40f,  // scurve_shadow
  1.60f,  // scurve_highlight
  0.47f,  // scurve_midpoint
  false,  // local_contrast_enabled
  0.30f,  // local_contrast
  12      // local_contrast_radius
};
...
//   FIT_COVER    -- resize to cover the whole panel; edges may be cropped
//
enum DisplayFit { FIT_ORIGINAL, FIT_CONTAIN, FIT_SCALE, FIT_COVER };
static const DisplayFit DISPLAY_FIT = FIT_COVER;

// Rotate portrait input images clockwise before layout, matching the PC script.
static const bool AUTO_ROTATE_PORTRAIT = true;
...
static const char* fit_name(DisplayFit f) {
  switch (f) {
    ...
    case FIT_COVER:    return "COVER";
  }
  return "?";
}

static bool rotate_portrait_clockwise(RgbImage* img) {
  if (!AUTO_ROTATE_PORTRAIT || !img || !img->pixels || img->height <= img->width) {
    return true;
  }
  ...
  const size_t bytes = (size_t)dst_w * dst_h * 3;
  uint8_t* dst = (uint8_t*)ps_malloc(bytes);
  if (!dst) dst = (uint8_t*)malloc(bytes);
  if (!dst) {
    LOG.println("[layout] portrait rotation OOM -- keeping original orientation");
    return false;
  }
  ...
  for (int y = 0; y < src_h; ++y) {
    for (int x = 0; x < src_w; ++x) {
      const size_t src = ((size_t)y * src_w + x) * 3;
      const size_t out = ((size_t)x * dst_w + (src_h - 1 - y)) * 3;
  ...
  img->width = dst_w;
  img->height = dst_h;
  LOG.printf("[layout] portrait image rotated clockwise -> %dx%d\n", dst_w, dst_h);
  return true;
}
...
static bool show_image_on_panel(RgbImage* img) {
  ...
  if (RAW_DITHER_ONLY) {
    LOG.println(TAG " raw dither only: image adjustment skipped");
  } else {
    LOG.printf(TAG " adjusting RGB image: exposure=%.2f saturation=%.2f vibrance=%.2f scurve=%s local_contrast=%s\n",
               IMAGE_ADJUST.exposure, IMAGE_ADJUST.saturation, IMAGE_ADJUST.vibrance,
               IMAGE_ADJUST.scurve_enabled ? "ON" : "OFF",
               IMAGE_ADJUST.local_contrast_enabled ? "ON" : "OFF");
    if (!adjust_rgb_image(img->pixels, W, H, IMAGE_ADJUST)) {
      LOG.println(TAG " image adjustment failed -- continuing with partially adjusted RGB");
    }
  }

  LOG.printf(TAG " dithering measured E6 with %s, gamma=%.2f, strength=%.2f ...\n",
             kDitherNames[(int)DITHER_METHOD], DITHER_GAMMA, DITHER_ERROR_STRENGTH);
  const uint32_t t0 = millis();
  if (!dither_image(img->pixels, W, H, PAL_E6, DITHER_METHOD,
                    DITHER_GAMMA, /*invert=*/false, idx,
                    DITHER_ERROR_STRENGTH)) {
...
void setup() {
  ...
  LOG.println();
  LOG.println("==============================================");
  LOG.println("  reTerminal E1002 -- Measured Palette Color6");
  LOG.println("==============================================");
